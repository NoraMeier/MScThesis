...
class Model:
    def __init__(self, local_rank, dropout_featextr=False, dropout_flowest=False, dropout_refine=False):
        ...
        # train
        self.optimG = AdamW(self.net.parameters(), lr=1e-2, weight_decay=1e-4)
        self.lap = LapLoss()
        #if local_rank != -1:
    ...
    def load_model(self, name=None, rank=0, custom=False):
        def convert(param):
            return {
            k.replace("module.", ""): v
        ...
        if custom:
            old_dict = torch.load(f'ckpt/{name}.pkl')
            new_dict = {"module." + key: val for key, val in old_dict.items()}
            self.net.load_state_dict(convert(new_dict))
            return
        if rank <= 0 :
            if name is None:
                name = self.name
